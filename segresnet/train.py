...
from monai.handlers import StatsHandler, TensorBoardStatsHandler, MLFlowHandler
...
# Custom conversion function to transform the label into multi‐channel format:
def convert_to_multichannel(label):
    import torch
    label_tensor = torch.tensor(label)
    # Tumor Core (TC): merge label 2 and 3
    tc = torch.logical_or(label_tensor == 2, label_tensor == 3)
    # Whole Tumor (WT): merge labels 1, 2 and 3
    wt = torch.logical_or(torch.logical_or(label_tensor == 1, label_tensor == 2), label_tensor == 3)
    # Enhancing Tumor (ET): label 2 only
    et = (label_tensor == 2)
    multi = torch.stack([tc, wt, et], dim=0).float()
    return multi

# A simple lambda wrapper to apply the conversion.
def convert_seg(x):
...
def main():
    ...
    # For validation, use an 80/20 split (adjust as needed)
    ...
    # Define transforms for images and segmentations.
    ...
    model = SegResNet(
        ...
        blocks_up=[1, 1, 1],
        init_filters=16,
        in_channels=4,   # expects a 4-channel input image (e.g. multi-modal)
    ...
    # Create the trainer and evaluator using Ignite.
    ...
    def run_validation(engine):
        ...
        dice = metrics["dice"]
        logging.info(f"Validation Dice: {dice:.4f}")
